...
///
/// Domain logic only. The two-tier (memory + disk) cache lives in the shared
/// `JSONDiskCache` actor; this type owns the network fetch, auth handoff, and
/// error mapping.
public actor SpotifyAnalysisClient {
    private let auth: SpotifyAuth
    private let cache = JSONDiskCache<String, SpotifyAudioAnalysis>(name: "SpotifyAnalysis")
    ...
    public func analysis(for trackID: String) async throws -> SpotifyAudioAnalysis {
        ...
        if let cached = await cache.get(trackID) {
            return cached
        }

        let fresh = try await fetchFromNetwork(trackID: trackID)
        await cache.put(trackID, fresh)
